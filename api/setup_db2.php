...
try {
    ...
    // 1. CLONIAMO LA STRUTTURA SENBIZ TOCCARE I DATI ORIGINALI
    ...
    if (!$check->fetch()) {
        try {
            // EXPLICT INSERT matching Fusion ERP structure entirely:
            // ts_users had: id, email, pwd_hash, role, full_name, password_changed_at, last_login_at, is_active, deleted_at, created_at, updated_at
            $insert = $db->prepare("INSERT INTO mv_users (id, email, pwd_hash, role, full_name, is_active) VALUES (?, ?, ?, 'admin', 'Admin MV Consulting', 1)");
